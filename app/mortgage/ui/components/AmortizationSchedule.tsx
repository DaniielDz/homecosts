...
interface AmortizationScheduleProps {
    ...
    firstPayment: string
    onFirstPaymentChange: (date: string) => void
}
...
export function AmortizationSchedule({
    ...
    // Last payment computed from last entry
    const lastPayment = useMemo(() => {
        if (!schedule.length) return ""
        const last = schedule[schedule.length - 1].date
        return new Date(`${last}-01`).toLocaleString("en-US", {
            month: "short",
            year: "numeric",
        })
    }, [schedule])
    ...
    return (
        <div className="w-full border-t-2 border-gray-200 mt-10 pt-6">
            ...
            <p className="text-sm text-gray-500 font-semibold mt-2 mb-6">
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between">
                <div className="flex items-end gap-5 ">
                    <div className="relative h-full">
                        <label className="flex flex-col gap-2 text-sm text-gray-900 font-medium" htmlFor="firstPayment">First Payment
                            <input
                                id="firstPayment"
                                type="month"
                                value={firstPayment}
                    ...
                    <div className="h-full flex flex-col justify-between gap-3">
                        <label className="text-sm text-gray-900 font-medium">Last Payment</label>
                ...
                <label className="relative mt-10 lg:mt-0 inline-flex items-center cursor-pointer">
                    <span className="mr-3 text-sm text-gray-900">Expand all years</span>
                    <input
                        type="checkbox"
                        checked={expandAll}
                    ...
                    {/* Thumb */}
                    <span className="absolute left-[75%] top-[2px] w-5 h-5 bg-white rounded-full shadow transform peer-checked:translate-x-5 transition-transform duration-200"></span>
                </label>
            ...
            <div className="mt-6 overflow-x-auto">
                <table className="w-full table-fixed">
                    <thead>
                        <tr className="text-sm text-gray-900 border-b border-gray-300">
                    ...
                    <tbody>
                        {[...grouped.entries()].map(([year, entries]) => {
                            // summary aggregates
                            const totalPrincipal = entries.reduce((a, e) => a + e.principal, 0)
                            const totalInterest = entries.reduce((a, e) => a + e.interest, 0)
                            ...
                            return (
                                <React.Fragment key={year}>
                                    <tr className="border-b border-gray-200 hover:bg-gray-50 cursor-pointer" onClick={() => toggleYear(year)}>
                                        <td className="py-5 text-blue-600 text-sm font-medium">{isExpanded ? "- " : "+ "}{year}</td>
                                        <td className="text-center py-5 text-sm text-gray-700">
                                            ${formatNumber(totalPrincipal, 2)}
                                        </td>
                                        <td className="text-center py-5 text-sm text-gray-700">
                                            ${formatNumber(totalInterest, 2)}
                                        </td>
                                        <td className="text-right py-5 text-sm text-gray-700">
                                    ...
                                    {isExpanded && entries.map((e) => {
                                        ...
                                        return (
                                            <tr key={e.date} className="border-b border-gray-100">
                                                ...
                                                <td className="text-center py-2 text-sm text-gray-700">
                                                    ${formatNumber(e.principal, 2)}
                                                </td>
                                                <td className="text-center py-2 text-sm text-gray-700">
                                                    ${formatNumber(e.interest, 2)}
                                                </td>
                                                <td className="text-right py-2 text-sm text-gray-700">
